...
public class Galeria extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnPick.setOnClickListener(v -> pickImageIntent());
        btnAntes.setOnClickListener(v -> {
            if(position > 0)
            {
                position--;
            }else{
    ...
    private void pickImageIntent(){
        ...
        intentLaucher.launch(Intent.createChooser(intent, "Selecciona las fotos"));
    }
                    ...
                    if(result.getResultCode()== Activity.RESULT_OK)
                    {
                        ...
                        if(result.getData().getClipData() != null)
                        {
                            ...
                            if(count < 16 && count > 10)
                            {
                                ...
                                if(count > 15)
                                    Toast.makeText(Galeria.this, "Máximo son 15 fotos", Toast.LENGTH_SHORT).show();
                                if(count < 10)
                                    Toast.makeText(Galeria.this, "Mínimo son 10 fotos", Toast.LENGTH_SHORT).show();
                            }
                        }else{
